...
def process(input_type='train', merge_with=None):
    #if test data merge_with=train
    ...
    if merge_with:
        keep_key = df_p_dyn.loc[:,['date', 'Person.ID']].drop_duplicates()
        df_p_dyn.append(pd.read_csv( '../data/'+ merge_with + '_person_dyn_raw.csv',low_memory=False, parse_dates=['date']))
    ...
    if merge_with:
        df_p_dyn_mod.merge(keep_key, how='inner', on=['Person.ID','date'])

    # we save the output as a csv
    df_p_dyn_mod.to_csv('../data/' + input_type + '_person_dyn_mod.csv',index = False)
    print('mod persons dyn on {} done'.format(input_type))
